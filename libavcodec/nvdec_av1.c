...
static int nvdec_av1_start_frame(AVCodecContext *avctx, const uint8_t *buffer, uint32_t size)
{
    ...
    *pp = (CUVIDPICPARAMS) {
        ...
        .CodecSpecific.av1 = {
            ...
            .coded_lossless               = frame_header->coded_lossless,
            ...
            /* SkipModeFrames */
            .SkipModeFrame0 = frame_header->skip_mode_frame_idx[0],
            .SkipModeFrame1 = frame_header->skip_mode_frame_idx[1],
            ...
            .loop_filter_mode_deltas[0] = s->cur_frame.loop_filter_mode_deltas[0],
            .loop_filter_mode_deltas[1] = s->cur_frame.loop_filter_mode_deltas[1],
    ...
    for (i = 0; i < AV1_MAX_SEGMENTS; ++i) {
        ...
        for (j = 0; j < AV1_SEG_LVL_MAX; ++j) {
            ppc->segmentation_feature_mask[i] |= s->cur_frame.feature_enabled[i][j] << j;
            ppc->segmentation_feature_data[i][j] = s->cur_frame.feature_value[i][j];
        }
    }

    /* Loopfilter */
    for (i = 0; i < AV1_NUM_REF_FRAMES; ++i) {
        ppc->loop_filter_ref_deltas[i] = s->cur_frame.loop_filter_ref_deltas[i];
    }
    ...
    for (i = 0; i < AV1_REFS_PER_FRAME; ++i) {
        ...
        /* Global Motion */
        ppc->global_motion[i].invalid = frame_header->is_global[AV1_REF_FRAME_LAST + i]; //TODO: Is this right?
